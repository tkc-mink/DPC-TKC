/* เมนูโลโก้ + init ปิดท้าย + ชื่อไฟล์บนหัว + ออโต้เซฟ + เก็บแถบเครื่องมือ + เปิด global
  ...
  setInterval(function () {
    ...
    else { s.textContent = '✓ บันทึกแล้ว · ⏳ ' + mm + ':' + ss; s.classList.add('clean'); var sv = localStorage.getItem('xls2_savedAt'); s.title = sv ? 'บันทึกล่าสุด: ' + new Date(+sv).toLocaleString('th-TH') : 'บันทึกแล้ว'; }
