  ...
  function buildStatIconList() {
    ...
    host.innerHTML = defs.map(function (d, i) {
      return '<div class="stdef" draggable="true" data-key="' + d.key + '" data-i="' + i + '">' +
        ...
        '<button type="button" class="stdef-icon stdef-iconglyph" data-icon="' + (d.icon || '').replace(/"/g, '&quot;') + '" title="คลิกเพื่อเลือกไอคอน" style="color:#' + (d.color || '888888') + '">' + (d.icon ? (d.icon + '\uFE0E') : '＋') + '</button>' +
    ...
    host.onclick = function (e) {
      ...
      if (ib) {
        openEmojiPicker(ib, ib.dataset.icon || '', function (emo) {
          ib.dataset.icon = emo; ib.textContent = emo ? (emo + '\uFE0E') : '＋'; collect();
        });
      }
    };
