...
(function () {
  ...
  var FOLDER_NAME = 'DYNAMIC PRICE LIST (TKC)';  // โฟลเดอร์เฉพาะใน Drive (สร้างอัตโนมัติ)
  ...
  function download() {
    if (state.busy) return Promise.resolve();
    if (!confirm('ดึงข้อมูลล่าสุดจาก Google Drive มาแทนที่ในเครื่องนี้?\n\n⚠️ ข้อมูลในเครื่องนี้จะถูกเขียนทับด้วยข้อมูลจาก Drive\n(ระบบดาวน์โหลดไฟล์สำรองของเครื่องนี้ให้ก่อนอัตโนมัติ)')) return Promise.resolve();
    try {
