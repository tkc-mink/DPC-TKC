...
(function () {
  ...
  function parseBd(v) {
    ...
    if (typeof v === 'number' || /^\d+$/.test(String(v))) {
      return { w: (String(v) === '2' ? 2 : 1), style: 'solid', color: '000000' };
    }
    var p = String(v).split('|');
    return { w: (p[0] === '2' ? 2 : 1), style: (p[1] || 'solid'), color: (p[2] || '000000') };
  }
  ...
  // วาดขอบหนึ่งด้านของช่อง (เส้นเดี่ยว · หรือ double = สองเส้นขนาน)
  function edge(out, x0, y0, x1, y1, v, zoom, dark) {
    var o = parseBd(v); if (!o) return;
    var col = displayColor(o.color, dark);
    var sw = Math.max(1, Math.round(o.w * Math.max(1, zoom)));   // หนาขึ้นตามซูม แบบ Excel
    var horiz = (y0 === y1);
    if (o.style === 'double') {
      var g = Math.max(1, Math.round(zoom));                     // ระยะห่างสองเส้น
      if (horiz) { seg(out, x0, y0 - g, x1, y1 - g, col, 1, null); seg(out, x0, y0 + g, x1, y1 + g, col, 1, null); }
      else { seg(out, x0 - g, y0, x1 - g, y1, col, 1, null); seg(out, x0 + g, y0, x1 + g, y1, col, 1, null); }
      return;
    }
    seg(out, x0, y0, x1, y1, col, sw, dashArr(o.style, sw));
  }
  ...
  function draw(rootEl, doc, view) {
    ...
    for (var i = 0; i < cells.length; i++) {
      ...
      if (bd.t) edge(out, x0, y0, x1, y0, bd.t, zoom, dark);
      if (bd.b) edge(out, x0, y1, x1, y1, bd.b, zoom, dark);
      if (bd.l) edge(out, x0, y0, x0, y1, bd.l, zoom, dark);
      if (bd.r) edge(out, x1, y0, x1, y1, bd.r, zoom, dark);
    }
